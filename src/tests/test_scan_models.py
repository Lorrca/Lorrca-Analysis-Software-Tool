...
class TestScanModels(unittest.TestCase):
    ...
    def test_osmo_model_initialization(self):
        model = OsmoModel(data=DATA, metadata=OSMO_METADATA)
    ...
    def test_oxy_model_initialization(self):
        model = OxyModel(data=DATA, metadata=OXY_METADATA)
    ...
    def test_osmo_model_equality(self):
        model1 = OsmoModel(data=DATA, metadata=OSMO_METADATA)
        model2 = OsmoModel(data=DATA, metadata=OSMO_METADATA)

        # Models with the same data and metadata should be equal
        self.assertEqual(model1, model2)

        # Change data and check equality (models should not be equal)
        model3 = OsmoModel(data=ALT_DATA, metadata=OSMO_METADATA)
        self.assertNotEqual(model1, model3)

    # Test equality comparison (__eq__) for OxyModel instances
    def test_oxy_model_equality(self):
        model1 = OxyModel(data=DATA, metadata=OXY_METADATA)
        model2 = OxyModel(data=DATA, metadata=OXY_METADATA)

        # Models with the same data and metadata should be equal
        self.assertEqual(model1, model2)

        # Change data and check equality (models should not be equal)
        model3 = OxyModel(data=ALT_DATA, metadata=OXY_METADATA)
        self.assertNotEqual(model1, model3)
